...
const companyCreateSchema = Yup.object().shape({
    ...
    tax: Yup.number().required('Este campo, é obrigatório').positive('Este campo, não pode receber números abaixo de 0').integer("Este campo não pode incluir um ponto decimal"),
    workspace: Yup.string().required('Este campo, é obrigatório').max(30, 'Este campo deve ter no máximo 30 caracteres'),
