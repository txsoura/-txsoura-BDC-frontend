...
import companyUserCreateSchema from 'schemas/companyUserCreate';
...
const Index = () => {
  ...
  const create: SubmitHandler = useCallback(
    async data => {
      try {
        ...
        await companyUserCreateSchema.validate(data, { abortEarly: false });

        const {
          data: { message: response },
        } = await serviceAPI.post(`companies/${companyId}/users`, data);
  ...
  return (
    <>
      <Header title="Usuários da construtora" />
                  ...
                  <div className="grid grid-cols-6 gap-6">
                    <div className="col-span-6 sm:col-span-3">
                      <Select
                        name="user_id"
                        label="Usuário"
                        options={handleOptions(users)}
                      />
                    </div>

                    <div className="col-span-6 sm:col-span-3">
                      <Select name="role" label="Cargo" options={roleOptions} />
                    </div>
                  </div>
                </div>
