...
const Index = () => {
  ...
  const create: SubmitHandler<UserProps> = useCallback(async data => {
    try {
      formRef.current.setErrors({});
      await userCreateSchema.validate(data, { abortEarly: false });

      const response = await serviceAPI.post('users', data);

      toast()
        .success('Wow!', response.data ? response.data.message : 'Criado')
        .with({
          duration: 4000,
          speed: 1000,
          positionX: 'end',
