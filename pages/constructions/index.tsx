...
import StatusIcon from "/components/Construction/StatusIcon";
import getApiError from "/utils/isApiError";
import {withNavBarLayout} from "/components/withNavBarLayout";
import {ConstructionProps} from "/types/construction";

const Index = () => {
    const router = useRouter()
    const [constructions, setConstructions] = useState<ConstructionProps[]>([]);
    ...
    const getItems = useCallback(async () => {
        try {
            const response = await serviceAPI.get(`constructions?paginate=10&page=${currentPage}`);
    ...
    return (
        <>
            <Header title='Construções' button={true} buttonText='Criar' buttonHref='/constructions/create'/>
                    ...
                    <div className="flex justify-center py-2 align-middle inline-block min-w-auto sm:px-6 lg:px-8">
                        <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                {constructions.map((user) => (
                                    <tr key={user.id}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.id}</td>

                                        <td className="px-6 py-4 whitespace-nowrap"
                                            onClick={() => router.push("/constructions/" + user.id)}>
                                            <div className="flex items-center">
                                                <div className="ml-4">
                                                    <div
                                                        className="text-sm font-medium text-gray-900">{user.name}</div>
                                                    <div className="text-sm text-gray-500">
                                                        Inicio: <b>{new Intl.DateTimeFormat('pt-BR', {
                                                        dateStyle: 'short',
                                                    }).format(new Date(user.start_date))}
                                                    </b> | Fim: <b>{new Intl.DateTimeFormat('pt-BR', {
                                                        dateStyle: 'short',
                                                    }).format(new Date(user.end_date))}
                                                    </b>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">R$ {user.budget}</td>
                                                    ...
                                                    <Menu.Items
                                                        ...
                                                        <div className="py-1">
                                                            ...
                                                            <Menu.Item>
                                                                {({active}) => (
                                                                    <button
                                                                        onClick={() => router.push("/constructions/" + user.id + "/edit")}
                                                                        type="submit"
                                                                        className={classNames(
                                                                            active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                                                                            'block w-full text-left px-4 py-2 text-sm'
